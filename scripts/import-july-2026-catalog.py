...
def waterdrop_specs(handle, title, description):
    ...
    dims = {"X12":"46,2 × 15,9 × 42,5 cm","X16":"46,2 × 15,9 × 42,5 cm","X8":"46,2 × 15,9 × 42,5 cm",
            ...
            "A1":"46,5 × 19,8 × 43,3 cm","C1H":"39,6 × 27,7 × 43,2 cm"}
    specs = {
        "Modell": model,
        ...
        "RO-Leistung": f"{gpd} GPD" if gpd else "Herstellerangabe modellabhängig",
        ...
        "Reinwasser-Abwasser-Verhältnis": ratios.get(model, "Herstellerangabe auf der Produktseite"),
        ...
        "Zertifizierung": "NSF/ANSI-Angaben siehe offizielle Produktseite",
        "Abmessungen": dims.get(model, "Auf der offiziellen Produktseite veröffentlicht"),
...
products.append({
    ...
    "specs":{"Bauart":"Auftisch-Umkehrosmoseanlage","Filtration":"Umkehrosmose; Filterleistung laut Hersteller nach NSF/ANSI 58 geprüft","Mineralisierung":"Einsetzbarer Mineralfilter","UV-Schutz":"Doppelte Luma-UV-Lampe","Heißwasser":"Integriertes Heizmodul","Rohwassertank":"5 Liter","Reinwassertank":"1 Liter","Abmessungen":"24 × 43,5 × 38,5 cm","Gewicht":"7,2 kg","Standby-Leistung":"1 W","Filterbetrieb":"30 W","Heizleistung":"2.200 W","Installation":"Keine Festinstallation erforderlich","Wartung":"Wechselbare Filtermodule laut Herstellerplan"},
...
for p in products:
    schema={"@context":"https://schema.org","@type":"Product","name":f'{p["brand"]} {p["name"]}',"image":f'https://hitmoda.ru/{p["image"]}',"brand":{"@type":"Brand","name":p["brand"]},"category":p["category"],"description":p["summary"]}
...
for slug in ["arktisquelle","waterdrop","aqua-global","ho3-filter"]:
    ...
    page=f'''<!DOCTYPE html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{esc(brand["name"])} – Marke und Produkte</title><meta name="description" content="{esc(brand["description_de"])}"><link rel="stylesheet" href="../assets/css/core.css?v=20260723-catalog1"><link rel="stylesheet" href="../assets/css/brand.css?v=20260723-catalog1"><link rel="stylesheet" href="../assets/css/product.css?v=20260723-catalog1"><link rel="canonical" href="https://hitmoda.ru/brands/{slug}.html"></head><body class="wm-v2">{header()}<main><div class="container"><div class="breadcrumbs"><a href="../index.html">Home</a> / <a href="../brands.html">Marken</a> / {esc(brand["name"])}</div><section class="brand-hero"><div class="brand-hero-primary"><div class="brand-hero-logo"><img src="../{esc(brand["logo"])}" alt="{esc(brand["name"])} Logo"></div><div class="brand-hero-copy"><span class="badge">Wasserfiltration</span><h1>{esc(brand["name"])}</h1><p class="lead">{esc(brand["description_de"])}</p></div></div><aside class="brand-hero-details"><h2>Markenprofil</h2><dl><div><dt>Land</dt><dd>{esc(brand.get("country","—"))}</dd></div><div><dt>Modelle</dt><dd>{len(items)}</dd></div></dl><a class="btn primary" href="{esc(brand["website"])}" target="_blank" rel="nofollow noopener">Offizielle Website ↗</a></aside></section><section class="panel"><h2>Produkte in der Datenbank</h2><div class="product-grid">{''.join(card(p) for p in items)}</div></section></div></main>{footer()}</body></html>'''
...
sitemap=re.sub(r'<url><loc>https://hitmoda.ru/products/arktisquelle-direct-flow-premium.html</loc></url>\s*','',sitemap)
for url in ["https://hitmoda.ru/brands/ho3-filter.html"]+[f'https://hitmoda.ru/products/{p["slug"]}.html' for p in products]:
